...
bool AudioOutputI2S::ResendLastSample() { //resend last sample to allow pause without click
#ifdef ESP32  
  while  (i2s_write_bytes((i2s_port_t)portNo, (const char *)&last_s32, sizeof(uint32_t), 0)) {
...
void AudioOutputI2S::flush()
{
    ...
    int buffersize = I2S_DMA_BUFF_SIZE * this->dma_buf_count;
