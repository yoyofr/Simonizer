...
class AudioOutput
{
  public:
    ...
    virtual bool SetGain(float f) { if (f>4.0) f = 4.0; if (f<0.0) f=0.0; gainF8P8 = (uint8_t)(f*(1<<8)); return true; }
    virtual bool SetVolume(uint16_t vol) { gainF8P8 = vol; return true; }
  ...
  protected:
    ...
    uint8_t bps;
    uint8_t channels;
    uint16_t gainF8P8; // Fixed point 8.8
